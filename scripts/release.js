...
try {
	...
	if (config.hasBlocks) {
		...
		const buildScript =
			packageData.scripts && packageData.scripts['build:assets'] ? 'build:assets' : 'build';
		runOptional(npmCommand, ['run', buildScript], 'Asset build');
	}

	runOptional(npmCommand, ['run', 'i18n'], 'Translation files');
